...
This Barcode TSD project also defines four local mixed-stack subagent profiles under `.codex/agents/`:

| Subagent id | Prompt file | Purpose |
|---|---|---|
| `tsd-orchestrator` | `.codex/agents/tsd-orchestrator.toml` | Coordinates 1C + Android work, integrates results, and updates OpenSpec task state |
| `tsd-1c-developer` | `.codex/agents/tsd-1c-developer.toml` | Owns extension metadata and BSL backend work under `extension/**` |
| `tsd-kotlin-developer` | `.codex/agents/tsd-kotlin-developer.toml` | Owns native Android work under `android/**` |
| `tsd-reviewer` | `.codex/agents/tsd-reviewer.toml` | Reviews the current Barcode TSD diff without editing files |
