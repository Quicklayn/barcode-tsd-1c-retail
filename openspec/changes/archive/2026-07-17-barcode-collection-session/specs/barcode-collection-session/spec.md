...
- `RT3/Catalogs/СтруктурныеЕдиницы.xml` confirms `Справочник.СтруктурныеЕдиницы`, its `ТипСтруктурнойЕдиницы` attribute, and the `Склад` enum value used by Retail warehouse selections.
- `RT3/InformationRegisters/ШтрихкодыНоменклатуры.xml` and the current `extension/src/HTTPServices/BarcodeTSD/Ext/Module.bsl` confirm the resolved product type and UUID-shaped `itemRef` already returned by lookup.
- `extension/src/Configuration.xml` confirms extension prefix `BarcodeTSD_`, compatibility mode `8.3.27`, and extension-first placement.
- `docs/api/tsd-api.yaml` is the cross-stack contract and will define the exact submission payload and response.
- 1C MCP metadata, memory, and documentation tools were not exposed in this session; the read-only local XML/BSL export was used as evidence instead.
