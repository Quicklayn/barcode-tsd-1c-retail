...
Verified from local metadata: `РегистрНакопления.ЗапасыНаСкладах` provides the
balance resource and dimensions named in these requirements; the configured
warehouse constant is `BarcodeTSD_Склад`. MCP metadata tools are not exposed in
this session.
