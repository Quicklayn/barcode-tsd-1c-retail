...
- live HTTP smoke для `invalid_request` и `not_found`.
...
Live happy-path против 1С требует ИБ с данными: реальный или тестовый штрихкод
из `РегистрСведений.ШтрихкодыНоменклатуры`. Временная ИБ, созданная из `RT3/`,
содержит метаданные, но не содержит товарных данных.
